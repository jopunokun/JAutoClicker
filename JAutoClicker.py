...
class AutoClicker:
    def __init__(self):
        ...
        # ADDITION ROW 1
        # Click Method Frame
        self.click_method_frame = tk.Frame(self.root, bg='#292929', padx=10, pady=10)
        click_method_lbl = tk.Label(self.click_method_frame, text='Click Method', bg='#292929', fg='white', font=('Arial', 12))
        click_method_lbl.pack(side=tk.LEFT, anchor='w')
        self.click_method_var = tk.StringVar()
        self.click_method_var.set("Click")
        click_method_options = ["Click", "Hold"]
        self.click_method_dropdown = ttk.Combobox(self.click_method_frame, textvariable=self.click_method_var, values=click_method_options, state="readonly", width=22)
        self.click_method_dropdown.pack(side=tk.LEFT, anchor='w')
        self.click_method_dropdown.bind("<<ComboboxSelected>>", lambda e: self.click_method_frame.focus())

        # Keybind Button Frame (ADD ROW 1 COL 2)
        self.change_keybind_button = tk.Button(self.click_method_frame, text="Change Keybind", width=12, command=self.on_change_keybind_button_click)
        self.change_keybind_button.pack(side=tk.LEFT, anchor='e', padx=(22, 3))
        self.reset_keybind_button = tk.Button(self.click_method_frame, text='Reset Keybind', width=12, command=self.reset_keybind)
        self.reset_keybind_button.pack(side=tk.LEFT, anchor='e')
        self.click_method_frame.pack(anchor='w')

        # ROW 2
        # Click Type Frame (ROW 2 COL 1)
        self.row_2_frame = tk.Frame(self.root, bg='#292929', padx=10)
        ...
        click_type_lbl2 = tk.Label(self.keyboard_row_2_frame, text='Press Method', bg='#292929', fg='white', font=('Arial', 12))
        click_type_lbl2.pack(side=tk.LEFT, anchor='w')
        self.press_type_var2 = tk.StringVar()
        self.press_type_var2.set("Press")
        press_type_options2 = ["Press", "Hold"]
        self.click_type_dropdown2 = ttk.Combobox(self.keyboard_row_2_frame, textvariable=self.press_type_var2, values=press_type_options2, state="readonly", width=21)
        self.click_type_dropdown2.pack(side=tk.LEFT, anchor='w')
        self.click_type_dropdown2.bind("<<ComboboxSelected>>", lambda e: self.keyboard_row_2_frame.focus())
        ...
        # Keyboard ROW 3a
        # Press Type Frame (ROW 3 COL 1)
        self.keyboard_row_3a_frame = tk.Frame(self.root, bg='#292929', padx=10)
        click_type_lbl = tk.Label(self.keyboard_row_3a_frame, text='Press Type', bg='#292929', fg='white', font=('Arial', 12))
        click_type_lbl.pack(side=tk.LEFT, anchor='w')
        ...
        self.click_type_dropdown = ttk.Combobox(self.keyboard_row_3a_frame, textvariable=self.press_type_var, values=press_type_options, state="readonly", width=21)
        self.click_type_dropdown.pack(side=tk.LEFT, anchor='w')
        self.click_type_dropdown.bind("<<ComboboxSelected>>", lambda e: self.keyboard_row_3a_frame.focus())
        ...
        if path.exists("assets/user_settings.json"):
            self.json_load()
        else:
            self.no_file = True
            self.json_save()
    ...
    def json_save(self):
        to_save = {
            ...
            'mouse_button_display': self.button_var.get(),
            'mouse_button': str(self.button),
            ...
            'keyboard_press_type2': self.press_type_var2.get(),
            'keyboard_key': str(self.keyboard_button),
            'click_method': str(self.click_method_var.get())
        }
    ...
    def json_load(self):
        ...
        self.button_var.set(data['mouse_button_display'])
        ...
        self.press_type_var2.set(data['keyboard_press_type2'])
        self.click_method_var.set(data['click_method'])
        self.change_pos()
        self.change_device()
    ...
    def change_device(self, event=None):
        if self.device_var.get() == 'Mouse':
            ...
            self.row_1_frame.pack(anchor='w')
            self.click_method_frame.pack(anchor='w')
            self.row_2_frame.pack(anchor='w', pady=(0, 10))
            self.row_3_frame.pack(anchor='w')
            self.click_pos_lbl.pack(side=tk.LEFT, anchor='w')
            self.change_pos()
            self.row_5_frame.pack(pady=(20, 0))
            ...
            self.click_method_frame.pack_forget()
            self.row_3_frame.pack_forget()
            self.default_frame.pack_forget()
            self.row_4_frame.pack_forget()
            ...
            if self.keybind == self.keyboard_button:
    ...
    def on_key_press(self, key):
        if self.keyboard_keybind_input.cget("text") != "Press any key to set keybind\nESC to cancel" or self.keybind_input.cget("text") != "Press any key to set keybind\nESC to cancel":
            self.keybind_od_before_kb = self.keyboard_keybind_input.cget("text")
            self.keybind_od_before_ms = self.keybind_input.cget("text")

        self.keyboard_key_before = self.keyboard_button
        #NON-CASE-SENSITIVE
        ...
        #BINDING KEY
            ...
            if key == Key.esc:
                self.is_binding_key = False
                self.keybind_input.config(text=self.keybind_od_before_ms, fg='white')
                self.keyboard_keybind_input.config(text=self.keybind_od_before_kb, fg='white')
                self.row_6_frame.pack_forget()
                ...
                self.warning_msg.config(text='Key CANNOT be the same as key to press', fg='red')
                self.row_6_frame.pack()
                self.keybind_input.config(text=self.keybind_od_before_kb, fg='white')
                self.keyboard_keybind_input.config(text=self.keybind_od_before_kb, fg='white')
        ...
        #RUN
            ...
            if self.status_lbl.cget('text') == 'Status: RUNNING':
                ...
                if self.press_type_var2.get() == 'Hold':
                    self.keyboard.release(self.keyboard_button)
                elif self.click_method_var.get() == 'Hold':
        ...
        #KEYBOARD KEY SET
            ...
            if key == Key.esc:
                ...
                self.row_6_frame.pack()
                self.keyboard_button = self.keyboard_key_before
    ...
    def run_clicker(self):
        ...
        while self.is_running:
            if self.device_var.get() == 'Mouse':
                if self.click_method_var.get() == 'Click':
                ...
                if self.press_type_var2.get() == 'Press':
                    if self.press_type_var.get() == 'Single Press':
                        ...
                        for i in range(3):
                            ...
                            self.keyboard.release(self.keyboard_button)
                        sleep(1/self.cps)
                elif self.press_type_var2.get() == 'Hold':
                    self.keyboard.press(self.keyboard_button)
                    break
        
        if self.click_method_var.get() == 'Hold':
            self.mouse.press(self.button)
